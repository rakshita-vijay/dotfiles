...
1. **_Only in this file, `GettingStarted-Basic.md` and `files/--HOME--/.shellrc` files (and nowhere else):_** Find and replace the strings that reference my usernames to your equivalent ones (for eg, you can search for `rakshita-vijay` (the `GH_USERNAME` env var) and `rakshita_vijay` (the `KEYBASE_USERNAME` env var) and replace them with your values).
